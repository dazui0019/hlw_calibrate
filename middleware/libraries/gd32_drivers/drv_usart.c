...
void DMA1_Channel2_IRQHandler(void)
{
    ...
    GD32_UART_DMA_IRQHandler(&serial0);

    /* leave interrupt */
    rt_interrupt_leave();
}
#endif /* defined(BSP_UART1_RX_USING_DMA) */
...
#endif /* defined(BSP_UART1_TX_USING_DMA) */
...
void DMA0_Channel5_IRQHandler(void)
{
    ...
    GD32_UART_DMA_IRQHandler(&serial1);
...
void DMA0_Channel1_IRQHandler(void)
{
    ...
    GD32_UART_DMA_IRQHandler(&serial2);

    /* leave interrupt */
    rt_interrupt_leave();
}
#endif /* defined(BSP_UART2_RX_USING_DMA) */
...
#endif /* defined(BSP_UART2_TX_USING_DMA) */
...
void DMA0_Channel6_IRQHandler(void)
{
    ...
    GD32_UART_DMA_IRQHandler(&serial7);
...
struct gd32_uart_dma uart0_rx_dma = {
    DMA1,
    DMA_CH2,
    DMA1_Channel2_IRQn,
    RCU_DMA1,
...
struct gd32_uart_dma uart0_tx_dma = {
    DMA1,
    DMA_CH7,
    DMA1_Channel7_IRQn,
    RCU_DMA1,
...
struct gd32_uart_dma uart1_rx_dma = {
    DMA0,
    DMA_CH5,
    DMA0_Channel5_IRQn,
    RCU_DMA0,
...
struct gd32_uart_dma uart1_tx_dma = {
    DMA0,
    DMA_CH6,
    DMA0_Channel6_IRQn,
    RCU_DMA0,
...
struct gd32_uart_dma uart2_rx_dma = {
    DMA0,
    DMA_CH1,
    DMA0_Channel1_IRQn,
    RCU_DMA0,
...
struct gd32_uart_dma uart2_tx_dma = {
    DMA0,
    DMA_CH3,
    DMA0_Channel3_IRQn,
    RCU_DMA0,
...
struct gd32_uart_dma uart7_rx_dma = {
    DMA0,
    DMA_CH6,
    DMA0_Channel6_IRQn,
    RCU_DMA0,
...
struct gd32_uart_dma uart7_tx_dma = {
    DMA0,
    DMA_CH0,
    DMA0_Channel0_IRQn,
    RCU_DMA0,
...
static const struct gd32_uart uart_obj[] = {
    #ifdef BSP_USING_UART0
    {
        ...
        DMA_CHXCTL_FTFIE | DMA_CHXCTL_HTFIE,
#endif
#if defined BSP_UART0_RX_USING_DMA
        &uart0_rx_dma,
#else
    ...
    #ifdef BSP_USING_UART1
    {
        ...
        RCU_GPIOD, RCU_GPIOD,                   // tx gpio clock, rx gpio clock
    ...
    #ifdef BSP_USING_UART5
    {
        ...
        RCU_USART5,                             // periph clock
#if defined SOC_SERIES_GD32F4xx
        RCU_GPIOA, RCU_GPIOA,                    // tx gpio clock, rt gpio clock
        GPIOA, GPIO_AF_8, GPIO_PIN_11,           // tx port, tx alternate, tx pin
        GPIOA, GPIO_AF_8, GPIO_PIN_12,           // rx port, rx alternate, rx pin
#elif defined SOC_SERIES_GD32E50x
        RCU_GPIOC, RCU_GPIOC,                    // tx gpio clock, rt gpio clock
...
#if defined(RT_SERIAL_USING_DMA) 
static void gd32_rx_dma_config(struct gd32_uart *uart)
{                    
    ...
    if(uart->dma_rx != 0){
        /* enable DMA */
        rcu_periph_clock_enable(uart->dma_rx->per_clk);  
        ...
        /* configure DMA mode */
        dma_circulation_enable(uart->dma_rx->dma_periph, uart->dma_rx->channel); 
        dma_channel_subperipheral_select(uart->dma_rx->dma_periph, uart->dma_rx->channel, uart->dma_rx->subperipheral);
        ...
        dma_interrupt_enable(uart->dma_rx->dma_periph, uart->dma_rx->channel, uart->dma_int_type);
...
static rt_err_t gd32_uart_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CONFIG:
        if (RT_DEVICE_FLAG_DMA_RX == ctrl_arg){
            gd32_rx_dma_config(uart);
        }else{
            gd32_uart_configure(serial, (struct serial_configure*)arg);
        }
        break;
	#endif
    }
 ...
 * @param[in]   size dma缓冲区已经使用的大小, 如果传入0, 则代表是空闲中断调用的
*/
#if defined(RT_SERIAL_USING_DMA)
void COMM_RxEventCallback(struct rt_serial_device *serial, uint32_t Size)
{
    ...
    Rx_length = Size - uart->dma_rx->last_index;

    rt_hw_serial_isr(serial, RT_SERIAL_EVENT_RX_DMADONE|(Rx_length << 8));

    uart->dma_rx->last_index += Rx_length;

    if (uart->dma_rx->last_index >= uart->serial->config.bufsz) uart->dma_rx->last_index = 0;
}
#endif
...
static void GD32_UART_IRQHandler(struct rt_serial_device *serial)
{
    ...
    if(usart_interrupt_flag_get(uart->uart_periph, USART_INT_FLAG_IDLE) != RESET){
        USART_STAT0(uart->uart_periph);
        USART_DATA(uart->uart_periph);
        COMM_RxEventCallback(serial, (uart->serial->config.bufsz - (uint32_t)DMA_CHCNT(uart->dma_rx->dma_periph, uart->dma_rx->channel)));
    }
#endif
}
...
static void GD32_UART_DMA_IRQHandler(struct rt_serial_device *serial)
{
    ...
    /* UART in mode Receiver -------------------------------------------------*/
    if(dma_interrupt_flag_get(uart->dma_rx->dma_periph, uart->dma_rx->channel, DMA_INTF_FTFIF)){
        dma_interrupt_flag_clear(uart->dma_rx->dma_periph, uart->dma_rx->channel, DMA_INTF_FTFIF);
        COMM_RxEventCallback(serial, uart->serial->config.bufsz);
    }else if(dma_interrupt_flag_get(uart->dma_rx->dma_periph, uart->dma_rx->channel, DMA_INTF_HTFIF)){
        dma_interrupt_flag_clear(uart->dma_rx->dma_periph, uart->dma_rx->channel, DMA_INTF_HTFIF);
        COMM_RxEventCallback(serial, (uart->serial->config.bufsz)/2);
    }
}
#endif
...
int rt_hw_usart_init(void)
{
    ...
    for (i = 0; i < sizeof(uart_obj) / sizeof(uart_obj[0]); i++)
    {
        ...
        result = rt_hw_serial_register(uart_obj[i].serial,
                              ...
                              #endif
                              | RT_DEVICE_FLAG_INT_RX, // MSH组件会以RT_DEVICE_FLAG_INT_RX方式打开串口
                              (void *)&uart_obj[i]);
